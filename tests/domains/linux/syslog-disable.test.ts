...
import { parseSyslogDisable } from "../../../src/internal/parsers/linux/syslog-disable.js";
...
const MANIFEST_ID = "cli.linux.syslog.disable";
const SAMPLE_TEXT = "Linux syslog disabled.";

describe("cli.linux.syslog.disable operation", () => {
  it("(1) builds exactly one CommandFrame for the literal, argument-free command", () => {
    ...
    expect(singleFrameCommand(operation, undefined as never)).toBe("linux syslog disable");
  });

  it("(2) parses sample output into a minimal acknowledgement shape", () => {
    expect(parseSyslogDisable(SAMPLE_TEXT)).toEqual({ raw: SAMPLE_TEXT });
  });
