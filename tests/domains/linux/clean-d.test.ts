...
import { parseCleanD } from "../../../src/internal/parsers/linux/clean-d.js";
...
const MANIFEST_ID = "cli.linux.clean.d";
const SAMPLE_TEXT = "Local data partition cleaned.";

describe("cli.linux.clean.d operation", () => {
  it("(1) builds exactly one CommandFrame for the literal, argument-free command", () => {
    ...
    expect(singleFrameCommand(operation, undefined as never)).toBe("linux clean -d");
  });

  it("(2) parses sample output into a minimal acknowledgement shape", () => {
    expect(parseCleanD(SAMPLE_TEXT)).toEqual({ raw: SAMPLE_TEXT });
  });
