...
import { parseCleanW } from "../../../src/internal/parsers/linux/clean-w.js";
...
const MANIFEST_ID = "cli.linux.clean.w";
const SAMPLE_TEXT = "All Linux service data wiped. Rebooting router.";

describe("cli.linux.clean.w operation (destructive)", () => {
  it("(1) builds exactly one CommandFrame for the literal, argument-free command", () => {
    ...
    expect(singleFrameCommand(operation, undefined as never)).toBe("linux clean -w");
  });

  it("(2) parses sample output into a minimal acknowledgement shape", () => {
    expect(parseCleanW(SAMPLE_TEXT)).toEqual({ raw: SAMPLE_TEXT });
  });
