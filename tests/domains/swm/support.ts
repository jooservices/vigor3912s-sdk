/**
 * Shared test support for `tests/domains/swm/*.test.ts` -- the fixed
 * four-test convention's manifest-linkage step (`ARCHITECTURE.md` Item 5).
 * Not a test file itself (no `.test.ts` suffix), scoped entirely within
 * this family's own write scope (`tests/domains/swm/**`). Mirrors
 * `tests/domains/wan/support.ts` (same shape, adapted to this family's own
 * relative import depth, which happens to be identical).
 */
