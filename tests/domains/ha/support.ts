/**
 * Shared test support for `tests/domains/ha/*.test.ts` -- the fixed
 ...
 * family's own write scope (`tests/domains/ha/**`). Mirrors
 * `tests/domains/wan/support.ts`'s identical helpers verbatim (this
 * family's write scope excludes shared test-support files, so it is
 * duplicated locally rather than imported cross-family).
 */
