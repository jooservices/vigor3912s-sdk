/**
 * Shared test support for `tests/domains/vigbrg/*.test.ts` -- the fixed
 ...
 * family's own write scope (`tests/domains/vigbrg/**`). Mirrors the sibling
 * `tests/domains/wan/support.ts` (duplicated deliberately, not imported --
 * each family's test scope stays self-contained).
 */
