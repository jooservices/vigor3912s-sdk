/**
 * Shared, non-test support module for `tests/domains/vigbrg/*.test.ts`'s
 * fourth test-convention step (round-trip through a fake transport and
 * surface closed-session failure). Not itself a `.test.ts` file, so Vitest
 * does not pick it up as a test suite. Mirrors
 * `tests/domains/sys/test-helpers.ts`'s `dispatchThroughFakeTransport`/
 * `expectClosedTransportFailure` pair, which route the assertion through the
 * real `DefaultCommandRunner` instead of calling a `FakeTransport` directly
 * (the latter only proves the fake's own hardcoded close-error message, not
 * the SDK runner's `session_closed` error-code wiring).
 */
