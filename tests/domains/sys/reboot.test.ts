...
const MANIFEST_ID = "cli.sys.reboot";
const operation = getSysOperation(MANIFEST_ID) as TypedOperation<void, SysAck>;

describe("sys reboot (destructive -- fake transport only, never invoked against anything real)", () => {
  it("builds the documented no-argument frame", () => {
    const [frame] = operation.buildFrames();

    expect(frameCommand(frame)).toBe("sys reboot");
  });
  ...
  it("dispatches through the fake transport and fails when the session is closed", async () => {
    ...
    expect(stdout).toBe("");
    await expectClosedTransportFailure("sys reboot");
  });
});
