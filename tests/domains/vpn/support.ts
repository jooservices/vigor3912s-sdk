/**
 * Shared test support for `tests/domains/vpn/*.test.ts` -- the fixed
 ...
 * family's own write scope (`tests/domains/vpn/**`). Mirrors
 * `tests/domains/wan/support.ts` (same helper shape, different domain).
 */
 ...
 * dangling successful dispatch. Mirrors
 * `tests/domains/sys/test-helpers.ts`'s `expectClosedTransportFailure`.
 */
