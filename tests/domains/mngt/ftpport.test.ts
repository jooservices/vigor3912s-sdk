...
const MANIFEST_ID = "cli.mngt.ftpport";
const operation = getMngtOperation(MANIFEST_ID) as TypedOperation<MngtPortInput, MngtAck>;

describe("mngt ftpport <port>", () => {
  it("builds the documented port frame", () => {
    const [frame] = operation.buildFrames({ port: 21 });

    expect(frameCommand(frame)).toBe("mngt ftpport 21");
  });

  it("rejects a non-integer and an out-of-range port", () => {
    expect(() => operation.buildFrames({ port: 1.5 })).toThrow(/integer between 1 and 65535/);
    expect(() => operation.buildFrames({ port: 70000 })).toThrow(/integer between 1 and 65535/);
  });
  ...
  it("dispatches through the fake transport and fails when the session is closed", async () => {
    const command = frameCommand(operation.buildFrames({ port: 21 })[0]);
