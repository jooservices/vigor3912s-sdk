...
const MANIFEST_ID = "cli.mngt.sslvpnport";
const operation = getMngtOperation(MANIFEST_ID) as TypedOperation<MngtPortInput, MngtAck>;

describe("mngt sslvpnport <port>", () => {
  it("builds the documented port frame", () => {
    const [frame] = operation.buildFrames({ port: 443 });

    expect(frameCommand(frame)).toBe("mngt sslvpnport 443");
  });

  it("rejects a non-integer and an out-of-range port", () => {
    expect(() => operation.buildFrames({ port: 1.1 })).toThrow(/integer between 1 and 65535/);
    expect(() => operation.buildFrames({ port: 100000 })).toThrow(/integer between 1 and 65535/);
  });
