/**
 * Shared test support for `tests/domains/ldap/*.test.ts` -- the fixed
 ...
 * family's own write scope (`tests/domains/ldap/**`). Mirrors
 * `tests/domains/wan/support.ts`'s precedent (own copy, no cross-family
 * import, per Wave 4's non-overlapping write-scope rule).
 */
