/**
 * Shared test support for `tests/domains/ip6/*.test.ts` -- the fixed
 ...
 * family's own write scope (`tests/domains/ip6/**`), mirroring the `wan`
 * family's own `support.ts` (deliberately not imported cross-family, per
 * Wave 4's non-overlapping write-scope rule).
 */
 ...
 * `TypedOperation` type). Mirrors `tests/domains/sys/test-helpers.ts`'s
...
/**
 * Test convention (4), failure path: a closed `FakeTransport` must cause the
 * runner to reject with `session_closed`, and must never leave a dangling
 * successful dispatch. Mirrors `tests/domains/sys/test-helpers.ts`'s
